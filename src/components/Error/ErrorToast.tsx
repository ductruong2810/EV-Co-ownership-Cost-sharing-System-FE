...
import { CheckCircleOutlined, CloseCircleOutlined, ExclamationCircleOutlined, WarningOutlined, InfoCircleOutlined, ReloadOutlined } from '@ant-design/icons'
...
export const showErrorToast = (error: ErrorInfo, options?: ToastOptions) => {
  ...
  return toast.error(
    <ErrorToastContent error={error} onRetry={error.action?.onClick} />,
    toastOptions
  )
}
...
export const showInfoToast = (message: string, title?: string) => {
  return toast.info(
    <div className='flex items-start gap-3 p-4 rounded-lg border-2 bg-white border-blue-400 shadow-xl min-w-[320px]'>
      <div className='flex-shrink-0 mt-0.5'>
        <InfoCircleOutlined className='text-blue-600 text-2xl font-bold drop-shadow-sm' style={{ filter: 'drop-shadow(0 2px 4px rgba(59, 130, 246, 0.3))' }} />
      </div>
      <div className='flex-1 min-w-0'>
...
export const showWarningToast = (message: string, title?: string) => {
  return toast.warning(
    <div className='flex items-start gap-3 p-4 rounded-lg border-2 bg-white border-yellow-400 shadow-xl min-w-[320px]'>
      <div className='flex-shrink-0 mt-0.5'>
        <WarningOutlined className='text-yellow-600 text-2xl font-bold drop-shadow-sm' style={{ filter: 'drop-shadow(0 2px 4px rgba(251, 191, 36, 0.3))' }} />
      </div>
      <div className='flex-1 min-w-0'>
        {title && <h4 className='font-bold text-gray-900 mb-1 text-sm'>{title}</h4>}
        <p className='text-sm font-medium text-gray-800'>{message}</p>
